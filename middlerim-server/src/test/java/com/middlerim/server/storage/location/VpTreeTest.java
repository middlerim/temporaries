...
public class VpTreeTest {
  ...
  public void testBuildAndFind() {
    ...
    int size = 10000;
    int i = 0;
    SphericalPoint p1 = new SphericalPoint(createDummySession(i++), 5.1, 5.1);
    points.add(p1);
    for (; i < size; ++i) {
      double d = (double) i;
      points.add(new SphericalPoint(createDummySession(i), 5.0999 + (d / 100000), 5.0999 + (d / 100000)));
    }
    for (int j = 0; j < 5; j++) {
  ...
  public void testNorthPole() {
    ...
    tree.put(new SphericalPoint(createDummySession(1), new Point(89.9, 70.9)));
    tree.put(new SphericalPoint(createDummySession(2), new Point(89.93109563906, 289.31721675720837)));
    tree.put(new SphericalPoint(createDummySession(3), new Point(89.93109563906, -290.55865037515025)));
    tree.put(new SphericalPoint(createDummySession(4), new Point(-89.93109563906, 70.9)));

    List<SphericalPoint> results = tree.findAround(createDummySession(1).sessionId, MessageCommands.areaKM(16));
    assertThat(results.size(), is(1));
    assertThat(results.get(0).point(), is(new Point(89.93109563906, -290.55865037515025)));
  }
